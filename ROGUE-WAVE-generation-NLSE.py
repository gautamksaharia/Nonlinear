# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 20:28:37 2023
...
def f(u, t, dx):
    ...
    ----------
    u : function
        u(x,t) is the function.
    t : float
        time.
    dx : float
        space between each spatial point.

    Returns
    -------
    
        real and imaginary part of Schrodinger equation.
...
def schrodinger(x0, xN, N, t0, tK, K):
    ...
    xN : final spatial point
    N : No of space points
    t0 : initial temporal point
    tK : final temporal point
    K : No of time pionts
    """
    ...
    U[0, :, 0] = 1  +  0.1*np.exp(-(x-0.1)**2/(120)) +  0.1*np.exp(-(x+0.1)**2/(120)) + 0.1*np.exp(-(x)**2/(120))
    ...
    plt.imshow(H[400:801:2, 1850:2151], interpolation='nearest', cmap='rainbow',
...
if __name__ == '__main__':
    schrodinger(-10, 10, 4001, 0, 10, 1200) # x0, xN, N, t0, tK, K